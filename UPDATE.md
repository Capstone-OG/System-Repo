...
- **Lưu trữ Hình ảnh Câu hỏi vào Database (Content Service)**:
  - Bổ sung thuộc tính `ImageUrl` vào `QuestionDto` trong `ImportMockExamCommand.cs`.
  - Cập nhật `ImportMockExamCommandHandler`: Tự động nhúng cú pháp Markdown `![Hình minh họa](image_url)` vào trường `ContentLatex` của câu hỏi khi lưu vào PostgreSQL Supabase, đảm bảo hiển thị đồng bộ trên mọi thiết bị học tập.
